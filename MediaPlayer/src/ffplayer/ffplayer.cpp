...
bool ffPlayer::startPlay(const std::string &filePath)
{
...
bool ffPlayer::stop(bool isWait)
{
    if (mPlayerState == VideoPlayer_Stop)
    {
        return false;
...
void ffPlayer::seek(int64_t pos)
{
    if(!seek_req)
    {
        seek_pos = pos;
...
void ffPlayer::setVolume(float value)
{
    mVolume = value;
}

double ffPlayer::getCurrentTime()
{
    return audio_clock;
}
...
void ffPlayer::readVideoFile()
{
	...
	emit sigTotalTimeChanged(getTotalTime());
...
bool ffPlayer::inputVideoQuene(const AVPacket &pkt)
{
...
bool ffPlayer::inputAudioQuene(const AVPacket &pkt)
{
    if (av_dup_packet((AVPacket*)&pkt) < 0)
    {
        return false;
...
///打开文件失败
void ffPlayer::doOpenVideoFileFailed(const int &code)
{
    fprintf(stderr, "%s \n", __FUNCTION__);
...
///打开sdl失败的时候回调此函数
void ffPlayer::doOpenSdlFailed(const int &code)
{
    fprintf(stderr, "%s \n", __FUNCTION__);
...
///获取到视频时长的时候调用此函数
void ffPlayer::doTotalTimeChanged(const int64_t &uSec)
{
    fprintf(stderr, "%s \n", __FUNCTION__);
...
///播放器状态改变的时候回调此函数
void ffPlayer::doPlayerStateChanged(const VideoPlayerState &state, const bool &hasVideo, const bool &hasAudio)
{
    fprintf(stderr, "%s \n", __FUNCTION__);
...
void ffPlayer::doDisplayVideo(const uint8_t *yuv420Buffer, const int &width, const int &height)
{
//    fprintf(stderr, "%s \n", __FUNCTION__);
    if (mVideoPlayerCallBack != nullptr)
    {
